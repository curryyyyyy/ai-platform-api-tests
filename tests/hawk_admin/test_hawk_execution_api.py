...
def test_hawk_06_01_start_init_batch(platform_client, platform_state):
    ...
    _assert_operation(platform_client, platform_state, "init", "HAWK_INIT_BATCH_ID", 1)


def test_hawk_06_02_pause_running_batch(platform_client, platform_state):
    """暂停处理中批次：对处理中批次执行 PAUSE 应成功。"""
    _assert_operation(platform_client, platform_state, "running", "HAWK_RUNNING_BATCH_ID", 2)


def test_hawk_06_03_restart_stopped_batch(platform_client, platform_state):
    """从已暂停批次重启：对已暂停批次执行 RESTART 应成功。"""
    _assert_operation(platform_client, platform_state, "stopped", "HAWK_STOPPED_BATCH_ID", 3)


def test_hawk_06_04_retry_failed_batch(platform_client, platform_state):
    """对失败批次执行重试：对失败批次执行 RETRY 应成功。"""
    _assert_operation(platform_client, platform_state, "failed", "HAWK_FAILED_BATCH_ID", 4)


def test_hawk_06_05_end_running_batch(platform_client, platform_state):
    """结束批次并验证终态：对处理中批次执行 END 应成功。"""
    _assert_operation(platform_client, platform_state, "running_end", "HAWK_END_RUNNING_BATCH_ID", 5)
...
def test_hawk_06_11_export_running_batch_preserves_status(platform_client, platform_state):
    ...
    batch_id = platform_state("running_with_tasks", "HAWK_RUNNING_BATCH_ID")
...
def test_hawk_06_06_operate_missing_batch(platform_client):
    ...
    body = assert_envelope(platform_client.operate_batch(999999999, 1), expected_code=None)
    assert body["code"] != 0
...
def test_hawk_06_07_batch_stat_missing_batch(platform_client):
    ...
    body = assert_envelope(platform_client.batch_stat(999999999), expected_code=None)
...
def test_hawk_06_12_batch_stats_preserves_valid_item(platform_client):
    ...
    body = assert_envelope(platform_client.batch_stats([999999999]), expected_code=None)
...
def test_hawk_06_09_fields_for_missing_batch_are_not_fabricated(platform_client):
    ...
    for response in (platform_client.key_fields(999999999), platform_client.output_fields(999999999)):
...
def test_hawk_06_10_error_log_for_missing_batch(platform_client):
    ...
    body = assert_envelope(platform_client.error_log(999999999), expected_code=None)
